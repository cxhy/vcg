...
- 默认自己完成架构分析和任务文档产出；后续设计、开发、测试、检查阶段必须按 `AGENTS.md`
  使用独立 subagent，并将 `reasoning_effort` 指定为 `high`。
...
```text
vcg-design（设计） -> vcg-python-dev（编码） -> vcg-python-tester（验证） -> vcg-verilog-checker（检查）
```

每步之间由用户确认或架构师检查后推进。你负责写任务单和最终架构检查，其他职责由独立
subagent 读取上游文档并执行，避免污染架构师上下文。
